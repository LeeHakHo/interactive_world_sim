...
| **up / down** | **coord2 ±(高度 z)** | 两视角以**竖直为主** + 小的**透视正确**水平分量 |

★**相机几何(实测)**:cam_high 光轴偏世界竖直 **24.1°**、cam_low **42.4°**(都非正 top-down)。
→ 纯 z 抬起在 cam_high 图像里不是「x 零位移」,而是竖直为主 + 小的透视正确水平分量(0.24m 抬起 → cam_high dx≈−11px, dy≈−32px;dy/dx≈3)。
- delta 作用在 eef3d 3 点**刚体星座质心**(星座内部偏移锁死),每按键 `REPEAT` 帧;每帧后世界系工作区 clip(BOX3D,防投影出画/OOD)。
- **真 3D 的修复 = 两视角是同一条 3D 轨迹的一致投影**(旧 bug=对两视角各打独立 image-y,不对应任何真实 3D 运动 → 虚假大水平串扰);纯 up/down 只改 coord2,反投影三角化回来是纯竖直 3D 运动。
- ⚠️**z 幅度上限**:cam_low 在 +0.5m 以上投影发散(点近相机)→ BOX3D z 上限 ≈ z0+0.28(约 0.38m 绝对),lift 幅度 ≤ ~0.25m。
