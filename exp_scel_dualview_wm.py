...
OUT = os.environ.get("OUT_DIR", "outputs/cross_embodiment_wm/dualview_wm")
...
class DualLWC(FlowWM_LWC):
    def __init__(s, P, **kw):
        ...
        s.act = nn.Linear((K + F) * 5 * 2 * 2, s.Dm)           # dummy5 x 2 views

    def fwd_dual(s, hist, eef_a, eef_b):
        """hist (B,2P,K,2) [view0 pts | view1 pts]; eef_a/b (B,K+F,3,2) per view."""
        B, P2 = hist.shape[:2]
        anchor = hist[:, :, -1, :]
        ...
        d5a = dummy5(eef_a) - oc_a[:, :, None]
        d5b = dummy5(eef_b) - oc_b[:, :, None]
...
def train_dual(trD, vsD, efA, efB, idx, seed=0, R_SS=32):
    torch.manual_seed(seed); P2 = trD.shape[2]
    m = DualLWC(P2 // 2, Dm=384, layers=3, W=15, vel_half=VEL_HALF).to(device)
...
def main():
    ...
    efA = z["eef"].astype(np.float32); efB = np.nan_to_num(z["eef_low"].astype(np.float32), nan=0.5)
    ...
    m = train_dual(trD, vsD, efA, efB, torch.from_numpy(pool))
    ...
    lift = zr > 0.08
    lines = [
        f"DUAL-VIEW ② (joint 2-view flow, dummy5 action) H={H} held-out n={len(ho)}",
        f"drift px cam_high: dual {float(eA.mean()):.2f}   (single-view 2D baseline 2.96, 3D 3.11)",
        f"drift px cam_low : dual {float(eB.mean()):.2f}   (single-view 2D baseline 3.10, 3D 7.78)",
        f"lift subset (n={int(lift.sum())}): high {float(eA[lift].mean()):.2f} | low {float(eB[lift].mean()):.2f}",
