...
def load_one(latp, condp):
    ...
    cond = np.load(condp)["sketch"]   # ★交互草图(sketch键), 非旧cond
...
def losses(model, z, cond, aux_tgt, n_robot):
    """返回 (main_loss, {name:aux_loss})。★main recon 只算 batch 里 robot 那 n_robot 个(human不进主头,
    避"输入robot输出human"矛盾); aux 算全 batch(robot+human 塑共享 trunk)。"""
    ...
    v_tgt = x1 - x0
    main = ((v_pred - v_tgt) ** 2)[:n_robot, :, :, 1:].mean()   # ★robot-only 主recon
...
def main():
    ...
    def batch(bs):
        ...
        return tt(z), tt(c), {name: tt(a[name]) for name in a}, n_robot

    run_m = run_a = 0.0
    for step in range(start_step + 1, STEPS + 1):
        z, c, at, nr = batch(BS)
        main_l, aux_l = losses(model, z, c, at, nr)
