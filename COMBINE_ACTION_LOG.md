...
## 4.6 stems 机制（EgoWAM 启发，2026-07-14 深夜，job 53318/53321）

读 EgoWAM（2607.08436）后补做的机制对照："每具身独立动作编码器 + 零显式对齐，共享 trunk 和
flow 目标隐式对齐"（他们的 HPT 式设计，我们此前机制族漏了这一格）。关键格 5 seeds 同 split 配对：

| v3 N=100 | rh | Δ | dual N=400 | rh | Δ |
|---|---|---|---|---|---|
| world/dummy5 锚 | 9.07±0.34 | +0.79 | | 3.44±0.52 | +1.34 |
| align_wm（显式教师）| **8.93±0.37** | **+1.39** | | 3.30±0.38 | +1.36 |
| stems（隐式）| 9.53±0.74 | +0.61 | | 3.39±0.30 | +1.39 |

**判决**：稀缺区（v3 N=100）隐式对齐**不够**——stems 配对差 +0.60（4/5 seed 同向，p=0.105），
还略差于单共享编码器；机制解释=独立 robot 编码器在 100 条数据下吃不饱梯度，EgoWAM 的隐式对齐
成立前提是他们的数据/骨干规模。充足区（dual N=400）三者打平（p=0.80）。→ **机制排序定稿：
稀缺区 显式冻结教师 > 单共享编码器 > 独立编码器隐式对齐 > 输入端改造(a1/warm) > 自蒸馏(有害)；
充足区全部收敛**。工程事故 ×2 记录：空 regs stack 崩（修+补测试）、_save 无锚 KeyError 崩
（值从 log 抢救，未损失任何训练）。
